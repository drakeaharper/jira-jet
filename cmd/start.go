...
var startCmd = &cobra.Command{
	...
	Short: "Start work on a ticket (transition to In Progress)",
	Long: `Start work on a JIRA ticket by transitioning it to "In Progress" status.

This is a shortcut for: jet shift TICKET-KEY "In Progress"
...
Example:
  jet start LX-123`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Find "In Progress" or similar transition
		var matchedTransition *jira.Transition
		progressStatuses := []string{"in progress", "in development", "progress"}
		...
		if matchedTransition == nil {
			...
			fmt.Printf("No 'In Progress' transition found for %s\n\n", ticketKey)
